...
export function PaginaPortarias() {
  ...
  async function carregar() {
    setCarregando(true);
    try {
      const r = await api.get("/api/Portaria/v1/listar/1/100");
  ...
  async function salvar(e: React.FormEvent) {
    ...
    try {
      await api.post("/api/Portaria/v1/salvar", { ...form, unidadeId: sessao?.unidadeId });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl">Portarias</h1>
        <Button onClick={() => setNovoOpen(true)}>Nova Portaria</Button>
      </div>
      ...
      <Card className="p-3">
        ...
        <div className="space-y-2">
          {itens.map((it: any) => (
            <div key={it.id ?? it.portariaId} className="p-2 hover:bg-muted rounded">
      ...
      <Dialog open={novoOpen} onOpenChange={setNovoOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Nova Portaria</DialogTitle></DialogHeader>
