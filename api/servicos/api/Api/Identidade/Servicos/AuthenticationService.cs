...
namespace Api.Identidade.Servicos
{
    public class AuthenticationService
    {
        ...
        private async Task<ClaimsIdentity> ObterClaimsUsuario(ICollection<Claim> claims, Usuario user)
        {
            ...
            claims.Add(new Claim("name", user.Nome ?? user.UserName));
            claims.Add(new Claim(PoliticasAuth.ClaimPapel, user.Papel.ToString()));
            if (user.UnidadeId.HasValue)
                claims.Add(new Claim(PoliticasAuth.ClaimUnidadeId, user.UnidadeId.Value.ToString()));
            if (user.PortariaPadraoId.HasValue)
                claims.Add(new Claim(PoliticasAuth.ClaimPortariaPadraoId, user.PortariaPadraoId.Value.ToString()));
            ...
            IEnumerable<Claim> claims, RefreshToken refreshToken)
        {
            return new UsuarioRespostaLogin
            {
                ...
                ExpiresIn = DataBrasilia.HorarioBrasilia().AddHours(8),
